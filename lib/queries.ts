...
export function byBA(f: Filter = ALL, limit: number | null = null) {
  ...
  return q<{ ba: string; revenue: number; qty: number; receipts: number }>(`
    ...
    order by revenue desc ${limit != null ? "limit $3" : ""}`,
    limit != null ? [f.months, f.source, limit] : [f.months, f.source]);
}
...
export type ShippedSku = { sku: string | null; product: string | null; size: string | null; barcode: string | null };
export type ReceiptPR = {
  id: number; po_number: string; order_date: string; units: number;
  lines: { id: number; scent: string; size: string; qty: number; barcode: string; grade: string | null }[];
  shipped_skus: ShippedSku[];
};
export async function pendingReceipts(userId: number, branch?: string): Promise<ReceiptPR[]> {
  const base = (whereExtra: string, arg: any) => q<ReceiptPR>(`
      select po.id, po.po_number, po.order_date::text order_date, coalesce(sum(i.qty),0)::float units,
             coalesce(json_agg(json_build_object('id', i.id, 'scent', i.scent, 'size', i.size, 'qty', i.qty, 'barcode', i.barcode,
                       'grade', (select p.grade from products p where p.barcode = i.barcode limit 1)) order by i.line_no)
  ...
  let rows: ReceiptPR[];
  try {
    rows = await base("po.assigned_to = $1", userId);
  } catch (e: any) {
    if (e?.code === "42703" && branch) {   // assigned_to column not migrated yet → old branch inbox
      try { rows = await base("upper(substring(po.branch_label from '^[0-9]+_([A-Za-z]+)')) = $1", normalizeBranch(branch)); }
      catch (e2: any) { if (e2?.code === "42P01" || e2?.code === "42703") return []; throw e2; }
    } else if (e?.code === "42P01" || e?.code === "42703") return [];
    else throw e;
  }
  // attach the warehouse-shipped serial SKUs (own guard: shipped_skus may not be migrated yet)
  if (rows.length) {
    try {
      const sk = await q<{ id: number; shipped_skus: ShippedSku[] | null }>(
        `select id, shipped_skus from purchase_orders where id = any($1)`, [rows.map((r) => r.id)]);
      const m = new Map(sk.map((r) => [r.id, r.shipped_skus ?? []]));
      for (const r of rows) r.shipped_skus = m.get(r.id) ?? [];
    } catch (e: any) { if (e?.code !== "42703") throw e; for (const r of rows) r.shipped_skus = []; }
  }
  return rows;
}
...
export function auditLog(filters: { action?: string; entity?: string; user?: string; from?: string; to?: string } = {}, limit = 200) {
  ...
  return q<{ id: number; user_name: string; user_role: string; action: string; entity: string; entity_id: string; detail: string; created_at: string }>(`
    ...
    where ($1::text is null or a.action = $1)
      ...
      and ($5::text is null or a.created_at::date >= $5::date)
      and ($6::text is null or a.created_at::date <= $6::date)
    order by a.created_at desc limit $4`,
    [filters.action || null, filters.entity || null, filters.user || null, limit, filters.from || null, filters.to || null]);
}
