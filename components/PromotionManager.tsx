          ...
          {grades.length === 0 ? <div className="text-sm text-muted py-4">ไม่พบเกรด/ขนาดในระบบสินค้า</div> : (
            <div className="overflow-x-auto">
              <table className="border-collapse">
                <thead><tr className="text-muted text-sm">
                  <th className="text-left px-3 pb-3 font-medium">เกรด</th>
                  {sizes.map((s) => <th key={s} className="px-3 pb-3 font-medium text-center whitespace-nowrap">{s}</th>)}
                </tr></thead>
                <tbody>
                  {grades.map((g) => (
