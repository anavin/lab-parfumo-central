...
export function DailyReport({ defaultSource = "CTW" }: { defaultSource?: string }) {
  ...
  return (
    <div className="card p-4 sm:p-5">
      ...
      <div className="rounded-xl border border-line bg-canvas px-4 py-4">
        ...
        {pending && !data ? (
          ...
          <>
            ...
            <Rule />
            <div className="space-y-1.5">
              <Line label="💵 เงินทอน" value={`${nf(change)} บาท`} />
      ...
      <p className="text-[11px] text-muted mt-2 text-center">เงินทอน = เงินสดรับ − ฝากเข้าธนาคาร · โอน/เครดิต = ทุกช่องทางที่ไม่ใช่เงินสด</p>
