...
function Panel({ open, onToggle, title, children }: { open: boolean; onToggle: () => void; title: string; children: React.ReactNode }) {
  return (
    <div className="mb-4">
      <button onClick={onToggle} className="px-3.5 py-2 rounded-lg bg-ink text-white text-sm font-medium hover:bg-black">
        {open ? "✕ ปิด" : title}
      </button>
  ...
  return (
    <Panel open={open} onToggle={() => setOpen((o) => !o)} title="+ เพิ่มรายการขาย">
      <div className="grid md:grid-cols-4 gap-3">
        ...
        <Field label="ช่องทาง"><select className={inp} value={f.source} onChange={(e) => s("source", e.target.value)}><option>CTW</option><option>EVENT_SCS</option></select></Field>
        ...
        <div className="md:col-span-4 flex items-center justify-between border-t border-black/5 pt-3">
          ...
          <button onClick={save} disabled={pending || !f.item} className="px-4 py-2 rounded-lg bg-ink text-white text-sm font-medium hover:bg-black disabled:opacity-50">บันทึกการขาย</button>
        </div>
      </div>
    </Panel>
  );
