...
export default async function MyPage({ searchParams }: { searchParams: Promise<{ date?: string }> }) {
  ...
  const [kpi, rows, trendRows, receipts, payRank, promo] = await Promise.all([
    ...
    getActivePromotionForSale(date),    // special prices in effect on this day (auto-fill at sale entry)
  ]);
